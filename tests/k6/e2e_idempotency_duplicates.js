...
export default function () {
  ...
  checkOrderState(confirmedOrder, { status: 'CONFIRMED', stockStatus: 'RESERVED', paymentStatus: 'PROCESSED' }, 'confirmed order');

  const duplicatePaymentRes = post(`${config.paymentsBaseUrl}/payments`, paymentPayload);
  checkStatus(duplicatePaymentRes, 200, 'duplicate payment');
  ...
  checkStatus(finalOrderRes, 200, 'fetch order after duplicate');
  const finalOrder = finalOrderRes.json();
  checkOrderState(finalOrder, { status: 'CONFIRMED', stockStatus: 'RESERVED', paymentStatus: 'PROCESSED' }, 'order after duplicate');
  check(finalOrder, {
    'order updatedAt unchanged after duplicate event': () => finalOrder.updatedAt === confirmedOrder.updatedAt
  });
}
